...
export function MaintenancePage({ config, embedded = false }: { config: MaintenanceConfig; embedded?: boolean }) {
  ...
  const page = <div
    ...
    className={cn(
      "overflow-y-auto bg-[#f7f9fc] text-slate-800",
      ...
      <section className="w-full min-w-0 overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_28px_80px_rgba(15,23,42,.10)]">
        ...
        <div className="p-5 sm:p-8 lg:p-10">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
            <span className="grid h-16 w-16 shrink-0 place-items-center rounded-2xl border border-slate-200 bg-white text-sky-600 shadow-sm"><Wrench className="h-7 w-7" /></span>
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-black uppercase tracking-[.2em] text-sky-600">Informasi layanan</p>
          ...
          <div className="mt-8 grid gap-3 border-t border-slate-200 pt-5 sm:grid-cols-[1fr_auto] sm:items-center">
            ...
            <a href="/login" className="inline-flex w-fit items-center gap-2 text-[11px] font-bold text-sky-700 transition hover:text-sky-800"><LockKeyhole className="h-3.5 w-3.5" />Akses Administrator</a>
          </div>
        </div>
      </section>
    </main>
