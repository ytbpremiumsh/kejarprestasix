  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
        ...
        ].map(({step,label,to,Icon}) => <Link key={to} to={to} className="group flex items-center gap-3 rounded-2xl border border-violet-100 bg-white p-3 shadow-sm transition hover:-translate-y-0.5 hover:border-violet-300 hover:shadow-md"><span className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 text-white shadow-sm shadow-violet-300"><Icon className="h-4 w-4"/></span><span className="min-w-0 flex-1"><span className="block text-[9px] font-black uppercase tracking-wider text-violet-600">Tahap {step}</span><span className="block truncate text-xs font-bold text-indigo-950">{label}</span></span><ArrowRight className="h-4 w-4 text-violet-300 transition group-hover:translate-x-0.5 group-hover:text-violet-600"/></Link>)}
      </div>
